...
export async function downloadAndExtractArchive(params: {
    ...
    onArchiveFile: (params: {
        ...
        writeFile: (params: { fileRelativePath: string; modifiedData?: Buffer }) => Promise<void>;
    }) => Promise<void>;
    cacheDirPath: string;
    ...
    await Promise.all(
        (await readdir(cacheDirPath))
            .filter(
                (() => {
                    const prefix = extractDirBasename.split("_").reverse().slice(1).reverse().join("_");
...
namespace SuccessTracker {
    ...
    async function write(params: { cacheDirPath: string; successTracker: SuccessTracker }) {
    ...
    export async function markAsExtracted(params: { cacheDirPath: string; extractDirBasename: string }) {
